...
document.addEventListener("DOMContentLoaded", () => {
    ...
    closeBtn?.addEventListener("click", closeGame);  //sends close signal and only closes on the close button
    ...
    function initializeSnakeGame() { //initializes the snake game, sets up the canvas and starts the game loop
        ...
        currentGame = new SnakeGame(gameContainer, {  //creates new instance of the game
            scale: 18,
            cols: 30,
            rows: 18,
        ...
        const wrapper = gameContainer.querySelector(".snake-wrapper") as HTMLElement | null;  //finds the wrapper element to add the start button to
        if (wrapper) {
    ...
    type Point = { x: number; y: number };  //simple type for points on the grid

    interface SnakeGameOptions {  //loads the options for laoding the gamne
        scale?: number;
        cols?: number;
        rows?: number;
        tickMs?: number;
    ...
    class SnakeGame {
        public onGameOver?: () => void;  //callback for when the game is over, will be used later

        private container: HTMLElement;  //main container for the game that will hold the canvas and timer
        private canvas: HTMLCanvasElement;  //canvas element where the game is rendered
        ...
        private boundKeyHandler = (e: KeyboardEvent) => this.handleKey(e);  //binding the key handler to the button events
        private boundVisibilityHandler = () => this.handleVisibilityChange();

        constructor(container: HTMLElement, options?: SnakeGameOptions) {  //constructor for the game 
            ...
            if (!ctx) throw new Error("Failed to get load canvas");  //erro handling incase canvas fails to load
            this.ctx = ctx;
        ...
        start() {
            ...
            if (this.running) return;  //prevents multiple starts if the button is pressed multiple times
        ...
        private startTimer() {  //starts the timer and updates the timer element every 250ms
            this.stopTimer();
        ...
        private stopTimer() {  //stops the timer and clears the interval
        ...
        private formatTime(ms: number) {  //formats the time in mm:ss format for display in the timer element
        ...
        private resetState() {  //resets the game state to the initial values and is used when starting a new game
        ...
        private gameOver() {  //handles the game over state
            this.running = false;
            if (this.intervalId !== null) {
                clearInterval(this.intervalId);
                this.intervalId = null;
            ...
            this.flashGameOver().then(async () => {  //flashes the screen red to indicate game over then shows the game over message and options
                ...
                try {
                    if (delta > 0) {
                        ...
                        if (userIdStr) {
                            const res = await (window as any).api.awardPoints(Number(userIdStr), delta, "snake");
                ...
                this.showGameMessage("Game Over", `You survived ${this.formatTime(elapsed)}. ${awardedText}`, [  //shows the game over message with the time survived and options to restart or quit
        ...
        private delay(ms: number) {
            return new Promise<void>(res => setTimeout(res, ms));  //utility function for creating delays which is used in the game over flash effect
        }

        private showGameMessage(titleText: string, bodyText: string, actions: { label: string, action: () => void }[]) {  //this is the styling for the game over messages
            ...
            let firstButton: HTMLButtonElement | null = null; //button styling
        ...
        private render() {  //renders the game state to the canvas
        ...
        private attachListeners() {  //attaches the event listeners for keyboard input and visibility change to handle pausing when the user switches tabs
        ...
        private handleKey(e: KeyboardEvent) {  //handles the keyboard input for controlling the snake ~supports both arrow keys and WASD~
            switch (e.key) {
                case "ArrowUp":
                case "w":
                case "W":
        ...
        private setNextDir(x: number, y: number) {  //sets the next direction of the snake based on user input
            if (this.dir.x === -x && this.dir.y === -y && this.snake.length > 1) return;
            this.nextDir = { x, y };
        }

        private pause() {  //pauses the game when the user switches tabs or the window loses focus to prevent the snake from running into walls while the user isn't looking
            if (this.intervalId !== null) {
                clearInterval(this.intervalId);
                this.intervalId = null;
            }
        ...
        private resume() { //game will resume when a user tabs back in
            if (!this.running) {
                this.running = true;
                if (this.intervalId === null) {
                    this.intervalId = window.setInterval(() => this.tick(), this.tickMs);
        ...
        private parseColor(input: string): { r: number; g: number; b: number } {  //parses the RBG colours so they can be used for shading in the snake canvas
            input = input.trim();
        ...
        private shadeRgb(c: { r: number; g: number; b: number }, percent: number) {
            const clamp = (v: number) => Math.max(0, Math.min(255, Math.round(v)));
            return { r: clamp(c.r + (255 - c.r) * percent), g: clamp(c.g + (255 - c.g) * percent), b: clamp(c.b + (255 - c.b) * percent) };
        }

        private rgbToCss(c: { r: number; g: number; b: number }) {
