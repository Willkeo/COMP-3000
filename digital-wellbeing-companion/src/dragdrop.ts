...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function closeGame() {
        ...
        if (gameContainer) gameContainer.innerHTML = "<p>Game content will load here</p>";
    }

    function initializeDragDropGame() {   //game loads
        stopTimer();
        ...
        uniqueTypes.forEach(type => {
            ...
            const sizeMap: Record<string, { w: number; h: number }> = {  //each drop zone will be sized to fit the shape and be as styled as acruatly as possible
            ...
            zone.addEventListener("drop", e => { //handles the drop event when a shape is dropped onto a drop zone
                e.preventDefault();
                ...
                if (shape.type === type) {
                    ...
                    (draggedEl as HTMLElement).style.width = "85%";  //scales the shape down slightly to fit better within the drop zone and look visually appealing
                    (draggedEl as HTMLElement).style.height = "85%";  //same as above but for height
    ...
    function startTimer(displayEl: HTMLElement | null) { //starts the countdown timer for the game
        stopTimer();
        remainingSeconds = DEFAULT_TIME_SECONDS;
        if (displayEl) displayEl.textContent = formatTime(remainingSeconds);  //updates the timer display with the initial time
        timerInterval = window.setInterval(() => {
            remainingSeconds -= 1;
            if (displayEl) displayEl.textContent = formatTime(remainingSeconds);  //updates the timer display every second
    ...
    function stopTimer() {  //stops the timer when the game is completed or when the user exits the game
    ...
    function formatTime(seconds: number) {  //adds the remaining time in minutes and seconds format to be displayed to the user
        const mm = Math.floor(seconds / 60).toString().padStart(2, "0");
        const ss = (seconds % 60).toString().padStart(2, "0");
        return `${mm}:${ss}`;
    }

    function onTimeUp() {  //message is displayed if the time runs out 
    ...
    async function checkCompletion() {
        if (placedCount >= totalTargets) {  //checks if all shapes have been placed correctly 
            ...
            let delta = 0;
            if (timeTaken <= 20) delta = 100;  //awards points based on how quickly the user completed the game
            else if (timeTaken <= 30) delta = 50;
            else if (timeTaken <= 60) delta = 20;
            ...
            try {
                if (delta > 0) {
                    ...
                    if (userIdStr) {
                        const res = await (window as any).api.awardPoints(Number(userIdStr), delta, "dragdrop");
            ...
            showGameMessage("Well done!", `Game completed in ${formatTime(timeTaken)}. ${awardedText}`, [  //shows completion message and the amount of points they earned
                { label: "Close", action: () => { closeGame(); } },
    ...
    function showGameMessage(titleText: string, bodyText: string, actions: { label: string, action: () => void }[]) {  //styling for the ingame messages
    ...
    function shuffleArray<T>(arr: T[]) {  //the function that randomises the order of shapes and dropzones
        const a = [...arr];
    ...
    function getPlaceholderSVG(type: string, w: number, h: number) {  //loads the SVGs to be user as placeholders for the dropzones
    ...
    function getShapeSVG(type: string, color: string, w: number, h: number) {  //loads the shape SVGs
        switch (type) {
            case "circle":
...
interface Shape {  //data model for the shapes used in the game
    id: string;
    type: string;
    color: string;
    complexity: number;
